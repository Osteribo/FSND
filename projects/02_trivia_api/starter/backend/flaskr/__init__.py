...
def create_app(test_config=None):
    ...
    def new_question():
        ...
        if (question and answer and category and difficulty) is None:
            abort(422)
    ...
    def search_questions():
        ...
        try:
            search_return = Question.query.order_by(
                                                    Question.id).filter(
                                                      Question.question.ilike(
                                                        f'%{search}%')).all()
            current_category = [question.category for question
                                in search_return]
    ...
    def play():
        ...
        try:
            ...
            filtered_options = [question.format() for question in
                                quiz_selections if question.id
